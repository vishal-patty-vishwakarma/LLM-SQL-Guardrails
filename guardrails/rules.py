...
def check_transaction_control(parsed: ParsedQuery) -> RuleResult:
    ...
    return RuleResult(passed=True, rule_id="G003", message="No transaction control")
...
def check_comments(parsed: ParsedQuery) -> RuleResult:
    ...
    return RuleResult(passed=True, rule_id="G005", message="No comments")
...
def check_single_statement(parsed: ParsedQuery) -> RuleResult:
    ...
    return RuleResult(passed=True, rule_id="G006", message="Single statement")
    ...
    for stmt in parsed.statements:
        ...
        if limit_node is None and isinstance(stmt, exp.Select):
            ...
            try:
                ...
                pass
    return RuleResult(passed=True, rule_id="G007", message="LIMIT valid")
...
def check_dangerous_functions(parsed: ParsedQuery) -> RuleResult:
    ...
    return RuleResult(passed=True, rule_id="G008", message="No dangerous functions")


def check_nesting_depth(parsed: ParsedQuery, max_depth: int = 5) -> RuleResult:
    if parsed.nesting_depth > max_depth:
        return RuleResult(passed=False, rule_id="G009",
    ...
    return RuleResult(passed=True, rule_id="G009", message="Nesting depth OK")


def check_join_tables(parsed: ParsedQuery, max_tables: int = 8) -> RuleResult:
    if len(parsed.tables) > max_tables:
        return RuleResult(passed=False, rule_id="G010",
    ...
    return RuleResult(passed=True, rule_id="G010", message="Table count OK")


DEFAULT_RULES: list[Rule] = [
    Rule(rule_id="G001", description="Block DDL/DML operations", check_fn=check_ddl_dml, action="block"),
    Rule(rule_id="G003", description="Block transaction control", check_fn=check_transaction_control, action="block"),
    Rule(rule_id="G005", description="Block SQL comments", check_fn=check_comments, action="block"),
    Rule(rule_id="G006", description="Require single statement", check_fn=check_single_statement, action="block"),
    Rule(rule_id="G008", description="Block dangerous functions", check_fn=check_dangerous_functions, action="block"),
    Rule(rule_id="G007", description="Require LIMIT clause", check_fn=check_limit, action="warn"),
    Rule(rule_id="G009", description="Check nesting depth", check_fn=check_nesting_depth, action="warn"),
    Rule(rule_id="G010", description="Check table count", check_fn=check_join_tables, action="warn"),
]